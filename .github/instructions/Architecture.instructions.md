...
└──────────────┬──────────────────────┘
               │ depends on
┌──────────────▼──────────────────────┐
│       Infrastructure                │  ← EF Core, PostgreSQL, Mapper
└──────────────┬──────────────────────┘
               │ depends on
┌──────────────▼──────────────────────┐
...
**Verantwortung:**
- Lokale Orchestrierung (Keycloak, PostgreSQL, API)
- Service Discovery
...
var database = postgres.AddDatabase("eletter25db");
...
- **Health Checks:** EF Core, PostgreSQL, Keycloak
...
- **.NET SDK:** 10.0
- **Docker:** Für PostgreSQL + Keycloak (via Aspire)
